 ...
 */
export function runExternalObjectLookupBenchmarks(numIterations, arraySize) {
  console.log(`\n[ EXTERNAL OBJECT LOOKUP BENCHMARK ]`)
  console.log(`${numIterations} iterations`)
  console.log(`${arraySize} array size`)

  const itemMap = new Map()
  for (let i = 0; i < arraySize; i++) {
    itemMap.set(i, squareEvenAndCubeOdd(i))
  }

  // forEach
  const forEachResult = runIterations(numIterations, arraySize, (data) => {
    const output = []
    data.forEach((item) => output.push(itemMap.get(item)))
    return output
  })
  ...
  const normalForResult = runIterations(numIterations, arraySize, (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      output.push(itemMap.get(data[i]))
    }
    return output
  })
  ...
  const mapResult = runIterations(numIterations, arraySize, (data) => {
    const output = data.map((item) => itemMap.get(item))
    return output
  })
